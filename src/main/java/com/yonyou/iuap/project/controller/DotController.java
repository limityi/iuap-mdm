 ...
 */
@RestController
@RequestMapping(value = "/dot")
public class DotController extends BaseController implements ServletContextAware {
    ...
    Object page(PageRequest pageRequest, SearchParams searchParams) {
        ...
        result.put("stationCompareTime", service.getSyncTime(RedisCacheKey.DOT_COMPARE_TIME));

        result.put("stationOnlyData", service.selectOnlyValidateByPage(pageRequestOnly, searchParams));
        result.put("stationOnlyTime", service.getSyncTime(RedisCacheKey.DOT_ONLY_TIME));

        result.put("stationRequiredData", service.selectRequiredData(pageRequestRequired, requiredColumn, searchParams));
        result.put("stationRequiredTime", service.getSyncTime(RedisCacheKey.DOT_REQUIRED_TIME));
    ...
    public @ResponseBody
    Object save(@RequestBody List<Dot> list) {
        service.save(list);
        return buildSuccess();
    }
    ...
    public @ResponseBody
    Object del(@RequestBody List<Dot> list) {
    ...
    public void exportExcel(HttpServletResponse response) {
        ...
        try {
            ...
            response.addHeader("Content-Disposition", "attachment; filename =" + URLEncoder.encode("网上飞网点数据质量报告.xlsx", "UTF-8"));
