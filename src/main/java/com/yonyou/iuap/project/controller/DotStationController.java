...
@RestController
@RequestMapping(value = "/DotStation") 
public class DotStationController extends BaseController implements ServletContextAware{

	@Autowired
	private DotStationService service;
    ...
    public void exportExcel(HttpServletResponse response) {
        ...
        try {
            //查询出全部数据
            Map<String,List<String>> dotstationMap=service.selectAllCacheForExcel();
            // 把数据写入到excel中，放到应用的临时路径下，再把这个文件传到浏览器
            String temppath = writeExcel.createExcelXlsx(dotstationMap,this.servletContext.getRealPath("/") + System.currentTimeMillis() + ".xlsx");
