...
public class OrgController extends BaseController implements ServletContextAware {
    ...
    Object page(PageRequest pageRequest, SearchParams searchParams) {
        ...
        result.put("stationCompareTime", service.getSyncTime(RedisCacheKey.ORG_COMPARE_TIME));

        result.put("stationOnlyData", service.selectOnlyValidateByPage(pageRequestOnly, searchParams));
        result.put("stationOnlyTime", service.getSyncTime(RedisCacheKey.ORG_ONLY_TIME));

        result.put("stationRequiredData", service.selectRequiredData(pageRequestRequired, requiredColumn, searchParams));
        result.put("stationRequiredTime", service.getSyncTime(RedisCacheKey.ORG_REQUIRED_TIME));
    ...
    public @ResponseBody
    Object save(@RequestBody List<Org> list) {
        service.save(list);
        return buildSuccess();
    }
    ...
    public @ResponseBody
    Object del(@RequestBody List<Org> list) {
    ...
    public void exportExcel(HttpServletResponse response) {
        ...
        try {
            ...
            response.addHeader("Content-Disposition", "attachment; filename =" + URLEncoder.encode("组织部门数据质量报告.xlsx", "UTF-8"));
