...
import com.yonyou.iuap.project.excel.WriteServiceAreaExcel;
import com.yonyou.iuap.project.excel.WriteStationExcel;
import com.yonyou.iuap.project.service.ServiceAreaService;
...
public class ServiceAreaController extends BaseController implements ServletContextAware {
    ...
    Object page(PageRequest pageRequest, SearchParams searchParams) {
        ...
        result.put("stationCompareTime", service.getSyncTime(RedisCacheKey.SERVICE_AREA_COMPARE_TIME));

        result.put("stationOnlyData", service.selectOnlyValidateByPage(pageRequestOnly, searchParams));
        result.put("stationOnlyTime", service.getSyncTime(RedisCacheKey.SERVICE_AREA_ONLY_TIME));

        result.put("stationRequiredData", service.selectRequiredData(pageRequestRequired, requiredColumn, searchParams));
        result.put("stationRequiredTime", service.getSyncTime(RedisCacheKey.SERVICE_AREA_REQUIRED_TIME));
    ...
    public @ResponseBody
    Object save(@RequestBody List<ServiceArea> list) {
        service.save(list);
        return buildSuccess();
    }
    ...
    public @ResponseBody
    Object del(@RequestBody List<ServiceArea> list) {
    ...
    public void exportExcel(HttpServletResponse response) {
        WriteServiceAreaExcel writeExcel = new WriteServiceAreaExcel();
        ...
        try {
            ...
            response.addHeader("Content-Disposition", "attachment; filename =" + URLEncoder.encode("服务区数据质量报告.xlsx", "UTF-8"));
