...
/**
 * 费用项目导出处理类
 * Created by zhugaofeng on 2018/12/22.
 *
 */
public class WriteCostitemExcel {
     ...
     * @return
     */
    public String createExcelXlsx(Map<String,List<String>> costitemMap, String path){
        // 创建文件输出流
        ...
        for(int i = 0; i < 5; i++){
            ...
            if(i == 0){
                cellRequired.setCellValue("费用项目-完整性校验结果");
            }
        }
        ...
        //格式化数据并填充
        List<String> compareData=costitemMap.get("compareData");
        this.setRowContent(sheet, rowIndex,compareData);

        List<String> onlyData=costitemMap.get("onlyData");
        this.setOnlyRowContent(sheetOnly, rowOnlyIndex,onlyData);

        List<String> requiredData=costitemMap.get("requiredData");
    ...
    private void setRowContent(XSSFSheet sheet, int rowIndex, List<String> data){
        if(!data.isEmpty()&&data.size()>0){
            for (int i=0;i<data.size();i++){
                Costitem costitem=gson.fromJson(data.get(i),Costitem.class);
