...
public class WriteNytLineExcel {
     ...
     * @return
     */
    public String createExcelXlsx(Map<String,List<String>> nytlineMap, String path){
        // 创建文件输出流
        ...
        for(int i = 0; i < 5; i++){
            ...
            if(i == 0){
                cell.setCellValue("南粤通客运线路 名称比较结果");
            }
        }
        for(int i = 0; i < 4; i++){
            ...
            if(i == 0){
                cellOnly.setCellValue("南粤通客运线路 未映射校验结果");
            }
        }
        /*for(int i = 0; i < 4; i++){
        ...
        for(int i = 0; i < 5; i++){
            ...
            switch(i){
                ...
                case 1 : setRowHeadValue(cell,"南粤通客运线路编码");break;
                case 2 : setRowHeadValue(cell,"南粤通客运线路名称");break;        
                case 3 : setRowHeadValue(cell,"客运线路名称");break;
                case 4 : setRowHeadValue(cell,"线路简称");break;
                default :break;
            }
        }
        ...
        for(int i = 0; i < 4; i++){
            ...
            switch(i){
                case 0 : setRowHeadValue(cellOnly,"南粤通客运线路编码");break;
                case 1 : setRowHeadValue(cellOnly,"主数据标准编码");break;
                case 2 : setRowHeadValue(cellOnly,"南粤通客运线路名称");break;
                case 3 : setRowHeadValue(cellOnly,"线路简称");break;
                default :break;
            }
        }
        ...
        //格式化数据并填充
        List<String> compareData=nytlineMap.get("compareData");
        this.setRowContent(sheet, rowIndex,compareData);

        List<String> onlyData=nytlineMap.get("onlyData");
        this.setOnlyRowContent(sheetOnly, rowOnlyIndex,onlyData);

        //List<String> requiredData=nytlineMap.get("requiredData");
    ...
    private void setRowContent(XSSFSheet sheet, int rowIndex, List<String> data){
        if(!data.isEmpty()&&data.size()>0){
            for (int i=0;i<data.size();i++){
                NytLine nytline=gson.fromJson(data.get(i),NytLine.class);

                XSSFRow row = sheet.createRow(rowIndex);

                this.setRowValue(row,0,nytline.getMcode());
                this.setRowValue(row,1,nytline.getCode());
                this.setRowValue(row,2,nytline.getName());
                this.setRowValue(row,3,nytline.getName1());
                this.setRowValue(row,4,nytline.getLine_shortname());
    ...
    private void setOnlyRowContent(XSSFSheet sheet, int rowIndex, List<String> data){
        if(!data.isEmpty()&&data.size()>0){
            for (int i=0;i<data.size();i++){
                ...
                this.setRowValue(row,0,nytline.getCode());
                this.setRowValue(row,1,nytline.getMcode());
                this.setRowValue(row,2,nytline.getName());
                this.setRowValue(row,3,nytline.getLine_shortname());
