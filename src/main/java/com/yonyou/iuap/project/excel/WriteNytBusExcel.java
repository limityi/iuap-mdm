...
public class WriteNytBusExcel {
     ...
     * @return
     */
    public String createExcelXlsx(Map<String,List<String>> nytbusMap, String path){
        // 创建文件输出流
        ...
        for(int i = 0; i < 5; i++){
            ...
            if(i == 0){
                cell.setCellValue("南粤通车辆 名称比较结果");
            }
        }
        for(int i = 0; i < 4; i++){
            ...
            if(i == 0){
                cellOnly.setCellValue("南粤通车辆 未映射校验结果");
            }
        }
        /*for(int i = 0; i < 4; i++){
        ...
        for(int i = 0; i < 5; i++){
            ...
            switch(i){
                ...
                case 1 : setRowHeadValue(cell,"南粤通车辆编码");break;
                case 2 : setRowHeadValue(cell,"南粤通车辆名称");break;        
                case 3 : setRowHeadValue(cell,"客运线路名称");break;
                case 4 : setRowHeadValue(cell,"车辆所属单位");break;
                default :break;
            }
        }
        ...
        for(int i = 0; i < 4; i++){
            ...
            switch(i){
                case 0 : setRowHeadValue(cellOnly,"南粤通车辆编码");break;
                case 1 : setRowHeadValue(cellOnly,"主数据标准编码");break;
                case 2 : setRowHeadValue(cellOnly,"南粤通车辆名称");break;
        ...
        //格式化数据并填充
        List<String> compareData=nytbusMap.get("compareData");
        this.setRowContent(sheet, rowIndex,compareData);

        List<String> onlyData=nytbusMap.get("onlyData");
        this.setOnlyRowContent(sheetOnly, rowOnlyIndex,onlyData);

        //List<String> requiredData=nytbusMap.get("requiredData");
    ...
    private void setRowContent(XSSFSheet sheet, int rowIndex, List<String> data){
        if(!data.isEmpty()&&data.size()>0){
            for (int i=0;i<data.size();i++){
                NytBus nytbus=gson.fromJson(data.get(i),NytBus.class);

                XSSFRow row = sheet.createRow(rowIndex);

                this.setRowValue(row,0,nytbus.getMcode());
                this.setRowValue(row,1,nytbus.getCode());
                this.setRowValue(row,2,nytbus.getName());
                this.setRowValue(row,3,nytbus.getName1());
                this.setRowValue(row,4,nytbus.getBus_depart());
    ...
    private void setOnlyRowContent(XSSFSheet sheet, int rowIndex, List<String> data){
        if(!data.isEmpty()&&data.size()>0){
            for (int i=0;i<data.size();i++){
                ...
                this.setRowValue(row,0,nytbus.getCode());
                this.setRowValue(row,1,nytbus.getMcode());
                this.setRowValue(row,2,nytbus.getName());
                this.setRowValue(row,3,nytbus.getBus_depart());
