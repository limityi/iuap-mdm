    ...
    public String createExcelXlsx(Map<String,List<String>> busMap, String path){
        ...
        for(int i = 0; i < 5; i++){
            ...
            switch(i){
                ...
                case 3 : setRowHeadValue(cell,"mdm编码");setRowHeadValue(cellOnly,cellRequired,"车辆号牌");break;       
                case 4 : setRowHeadValue(cell,"车辆号牌");break;
                default :break;
            }
        }
    ...
    private void setRowContent(XSSFSheet sheet, int rowIndex, List<String> data){
        if(!data.isEmpty()&&data.size()>0){
            for (int i=0;i<data.size();i++){
                ...
                this.setRowValue(row,1,bus.getSimilarity());
                this.setRowValue(row,2,bus.getName());
                this.setRowValue(row,3,bus.getMdm_code());                
                this.setRowValue(row,4,bus.getBus_lisencenum());                
