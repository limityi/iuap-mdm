...
public class WriteDotStationExcel {
     ...
     * @return
     */
    public String createExcelXlsx(Map<String,List<String>> dotstationMap, String path){
        // 创建文件输出流
        ...
        for(int i = 0; i < 4; i++){
            ...
            if(i == 0){
                cell.setCellValue("网上飞-站场 名称相似度结果");
            }
        }
        /*for(int i = 0; i < 4; i++){
            ...
            if(i == 0){
                cellOnly.setCellValue("南粤通站场 未映射校验结果");
            }
        }*/
        /*for(int i = 0; i < 4; i++){
        ...
        for(int i = 0; i < 4; i++){
            ...
            switch(i){
                case 0 : setRowHeadValue(cell,"网上飞网点编码");break;
                case 1 : setRowHeadValue(cell,"网上飞网点名称");break;  
                case 2 : setRowHeadValue(cell,"站场编码");break;
        ...
        /*for(int i = 0; i < 4; i++){
            ...
            switch(i){
                case 0 : setRowHeadValue(cellOnly,"南粤通站场编码");break;
                case 1 : setRowHeadValue(cellOnly,"主数据标准编码");break;
                case 2 : setRowHeadValue(cellOnly,"南粤通站场名称");break;
                case 3 : setRowHeadValue(cellOnly,"站场经营单位");break;
                default :break;
            }
        }*/
        ...
        //格式化数据并填充
        List<String> compareData=dotstationMap.get("compareData");
        this.setRowContent(sheet, rowIndex,compareData);

        //List<String> onlyData=dotstationMap.get("onlyData");
        //this.setOnlyRowContent(sheetOnly, rowOnlyIndex,onlyData);

        //List<String> requiredData=dotstationMap.get("requiredData");
    ...
    private void setRowContent(XSSFSheet sheet, int rowIndex, List<String> data){
        if(!data.isEmpty()&&data.size()>0){
            for (int i=0;i<data.size();i++){
                DotStation dotstation=gson.fromJson(data.get(i),DotStation.class);

                XSSFRow row = sheet.createRow(rowIndex);

                this.setRowValue(row,0,dotstation.getCode());
                this.setRowValue(row,1,dotstation.getName());
                this.setRowValue(row,2,dotstation.getStationcode());
                this.setRowValue(row,3,dotstation.getStationname());
    ...
    /*private void setOnlyRowContent(XSSFSheet sheet, int rowIndex, List<String> data){
        if(!data.isEmpty()&&data.size()>0){
            for (int i=0;i<data.size();i++){
            	NytStation nytstation=gson.fromJson(data.get(i),NytStation.class);

                XSSFRow row = sheet.createRow(rowIndex);

                this.setRowValue(row,0,nytstation.getCode());
                this.setRowValue(row,1,nytstation.getMcode());
                this.setRowValue(row,2,nytstation.getName());
                this.setRowValue(row,3,nytstation.getStation_companyname());
