    ...
    public String createExcelXlsx(Map<String,List<String>> serviceareaMap, String path){
        ...
        for(int i = 0; i < 18; i++){
            ...
            switch(i){
                ...
                case 2 : setRowHeadValue(cell,"名称");setRowHeadValue(cellOnly,cellRequired,"所属高速");break;
                case 3 : setRowHeadValue(cell,"所属高速");setRowHeadValue(cellOnly,cellRequired,"路段");break;
                case 4 : setRowHeadValue(cell,"路段");setRowHeadValue(cellOnly,cellRequired,"行政区属地");break;
                case 5 : setRowHeadValue(cell,"行政区属地");setRowHeadValue(cellOnly,cellRequired,"桩号");break;
                case 6 : setRowHeadValue(cell,"桩号");setRowHeadValue(cellOnly,cellRequired,"店长");break;
                case 7 : setRowHeadValue(cell,"店长");setRowHeadValue(cellOnly,cellRequired,"电话");break;
    ...
    private void setOnlyRowContent(XSSFSheet sheet, int rowIndex, List<String> data){
        if(!data.isEmpty()&&data.size()>0){
            for (int i=0;i<data.size();i++){
                ...
                this.setRowValue(row,2,servicearea.getHighspeedway()); 
                this.setRowValue(row,3,servicearea.getSectionid());
                this.setRowValue(row,4,servicearea.getAdministrativeregion());
                this.setRowValue(row,5,servicearea.getStakemark());
                this.setRowValue(row,6,servicearea.getShopmanager());                
                this.setRowValue(row,7,servicearea.getPhone());
                this.setRowValue(row,8,servicearea.getBusinesstype());
                this.setRowValue(row,9,servicearea.getYyowner());
                this.setRowValue(row,10,servicearea.getStartbusinessdate());
                this.setRowValue(row,11,servicearea.getOpendate());                
                this.setRowValue(row,12,servicearea.getGasflag());
                this.setRowValue(row,13,servicearea.getParkflag());
                this.setRowValue(row,14,servicearea.getCvsflag());
                this.setRowValue(row,15,servicearea.getCateringflag());
                this.setRowValue(row,16,servicearea.getRepairingflag());
