...
public class BusLineService {
    ...
    public Page<BusLine> selectAllByPage(PageRequest pageRequest, SearchParams searchParams) {
        ...
        if (updateOperation) {
            ...
            if (searchMap.get("searchParam").equals("null")) {
                ...
                pageResult = dao.selectCacheByCondition(pageRequest, RedisCacheKey.BUSLINE_COMPARE_DATA, searchMap);
            }
        }
        return pageResult;
    }
     ...
     * @param searchMap
     */
    private void similarityMatch(Page<BusLine> pageResult) {
        ...
        long resultCacheSize = redisTemplate.llen(RedisCacheKey.BUSLINE_COMPARE_DATA);
        overviewService.updateMdmDataStatistics(DTEnum.MdmSys.MDM.getId(), DTEnum.UserMenus.busline.getId().split("md_")[1].toUpperCase(), DTEnum.UserMenus.busline.getDtName(), 2, resultCacheSize);
    }
    ...
    public Page<BusLine> selectRequiredData(PageRequest pageRequest, List<String> requiredColumn, SearchParams searchParams) {
        ...
        boolean updateOperation = Boolean.parseBoolean(searchParams.getSearchMap().get("updateOperation").toString());
        Page<BusLine> pageResult = null;
        if (updateOperation) {
            boolean lock = RedisUtil.tryGetDistributedLock(redisTemplate, RedisCacheKey.BUSLINE_REQUIRED_DATA, requestId, RedisUtil.getLock_timeout());
            if (lock) {
                ...
                if (result > 0) {
                    //有数据设置同步时间
                    setSyncTime(RedisCacheKey.BUSLINE_REQUIRED_TIME);
                    RedisUtil.releaseDistributedLock(redisTemplate, RedisCacheKey.BUSLINE_REQUIRED_DATA, requestId);
            ...
            if (searchMap.get("searchParam").equals("null")) {
                ...
                if ((!pageResult.getContent().isEmpty()) && pageResult.getContent().size() > 0) {
                    ...
                    boolean lock = RedisUtil.releaseDistributedLock(redisTemplate, RedisCacheKey.BUSLINE_REQUIRED_DATA, requestId);
                    if (lock) {
                        //从数据库查询全部数据
                        //查询数据库数据量
                        int result = dao.selectRequiredData(requiredColumn, searchMap);
                ...
                pageResult = dao.selectCacheByConditionRequired(pageRequest, RedisCacheKey.BUSLINE_COMPARE_DATA, searchMap);
            }
        }
        return pageResult;
    }
    ...
    private void syncCacheData(String requestId) {
        ...
        boolean lock = RedisUtil.tryGetDistributedLock(redisTemplate, RedisCacheKey.BUSLINE_COMPARE_DATA, requestId, RedisUtil.getLock_timeout());
        ...
        if (lock) {
            ...
            //查询全部结果
            Page<BusLine> pageResult = dao.selectAllByPage(pageRequestTemp);
            //相似度比较
            ...
            //释放分布式锁
            RedisUtil.releaseDistributedLock(redisTemplate, RedisCacheKey.BUSLINE_COMPARE_DATA, requestId);
        }
    }
}
