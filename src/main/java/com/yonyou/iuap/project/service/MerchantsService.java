...
public class MerchantsService {
    ...
    private void similarityMatch(Page<Merchants> pageResult, Map<String, Object> searchMap) {
        ...
        //循环的list
        List<Merchants> stationList = pageResult.getContent();
        //匹配的list
        CopyOnWriteArrayList<Merchants> stationMatch = new CopyOnWriteArrayList<>(pageResult.getContent());
        //结果的list
        List<Merchants> stationResult = new ArrayList<>();
        ...
        if (object != null) {
            ...
            //循环判断
            for (Merchants merchants : stationList) {
                ...
                try {
                    ...
                    //判断结果集是否已经包含
                    if (!contain(stationResult, merchants)) {
                        //反射取字段值
                        Method method = merchants.getClass().getMethod(matchField, null);
                        String field1 = (String) method.invoke(merchants);

                        //循环去匹配
                        for (Merchants sta : stationMatch) {
                            //如果不是同一条数据则进行匹配
                            if (!merchants.getMdm_code().equals(sta.getMdm_code())) {
                                ...
                                if (similarity > 0.9) {
                                    ...
                                    //同时将数据写入redis
                                    redisTemplate.rpush(RedisCacheKey.MERCHANTS_COMPARE_DATA, gson.toJson(sta));
    ...
    private boolean contain(List<Merchants> list, Merchants merchants) {
        ...
        if (list != null && list.size() > 0 && merchants != null) {
            for (Merchants sta : list) {
                //比较id
                if (sta.getId().equals(merchants.getId())) {
                    result = true;
                    break;
                }
            }
