...
public class StoresService {
    ...
    public Page<Stores> selectAllByPage(PageRequest pageRequest, SearchParams searchParams) {
        ...
        if (updateOperation) {
            ...
            //比较完之后再从缓存取值
            pageResult = dao.selectAllByCache(pageRequest, RedisCacheKey.STORES_COMPARE_DATA);
        } else {
            if (searchMap.get("searchParam").equals("null")) {
                ...
                pageResult = dao.selectCacheByCondition(pageRequest, RedisCacheKey.STORES_COMPARE_DATA, searchMap);
            }
        }
        return pageResult;
    }

    public void save(List<Stores> recordList) {
        List<Stores> addList = new ArrayList<>(recordList.size());
        List<Stores> updateList = new ArrayList<>(recordList.size());
        for (Stores meta : recordList) {
            if (meta.getId() == null) {
    ...
    public void batchDeleteByPrimaryKey(List<Stores> list) {
     ...
     * @param searchMap
     */
    private void similarityMatch(Page<Stores> pageResult) {
        Map<String, Object> searchMap = new HashMap<>();
        //匹配之前，先删除redis数据
        redisTemplate.del(RedisCacheKey.STORES_COMPARE_DATA);
        //处理数据，相似度检查
        //根据查询的参数，看是哪个字段需要检查相似度
        //循环的list
        List<Stores> stationList = pageResult.getContent();
        //匹配的list
        CopyOnWriteArrayList<Stores> stationMatch = new CopyOnWriteArrayList<>(pageResult.getContent());
        //结果的list
        List<Stores> stationResult = new ArrayList<>();
        ...
        if (object != null) {
            ...
            for (Stores stores : stationList) {
                ...
                try {
                    ...
                    //判断结果集是否已经包含
                    if (!contain(stationResult, stores)) {
                        //反射取字段值
                        Method method = stores.getClass().getMethod(matchField, null);
                        String field1 = (String) method.invoke(stores);
        ...
        long resultCacheSize = redisTemplate.llen(RedisCacheKey.STORES_COMPARE_DATA);
        overviewService.updateMdmDataStatistics(DTEnum.MdmSys.MDM.getId(),"MDM_STORES", DTEnum.UserMenus.stores.getDtName(), 2, resultCacheSize);

    }

    private boolean contain(List<Stores> list, Stores stores) {
        boolean result = false;
        if (list != null && list.size() > 0 && stores != null) {
            for (Stores sta : list) {
                //比较id
                if (sta.getId().equals(stores.getId())) {
                    result = true;
                    break;
                }
            }
    ...
    public String getSyncTime(String fieldName) {
        return RedisUtil.getSyncTime(redisTemplate, RedisCacheKey.STORES_COMPARE_TIME, fieldName);
    }
    ...
    private void setSyncTime(String fieldName) {
        ...
        RedisUtil.setSyncTime(redisTemplate, RedisCacheKey.STORES_COMPARE_TIME, fieldName, format.format(new Date()));
    }
     ...
     * @return
     */
    public Page<Stores> selectOnlyValidateByPage(PageRequest pageRequest, SearchParams searchParams) {
        ...
        if (updateOperation) {
            ...
            if (result > 0) {
                //有数据设置同步时间
                setSyncTime(RedisCacheKey.STORES_ONLY_TIME);
                pageResult = dao.selectAllByCache(pageRequest, RedisCacheKey.STORES_ONLY_DATA);
            } else {
                setSyncTime(RedisCacheKey.STORES_ONLY_TIME);
                pageResult = new PageImpl<>(new ArrayList<Stores>(), pageRequest, 0);
            }
        } else {
            //查询缓存数据
            pageResult = dao.selectAllByCache(pageRequest, RedisCacheKey.STORES_ONLY_DATA);
    ...
    public Page<Stores> selectRequiredData(PageRequest pageRequest, List<String> requiredColumn, SearchParams searchParams) {
        ...
        boolean updateOperation = Boolean.parseBoolean(searchParams.getSearchMap().get("updateOperation").toString());
        Page<Stores> pageResult = null;
        if (updateOperation) {
            boolean lock = RedisUtil.tryGetDistributedLock(redisTemplate, RedisCacheKey.STORES_REQUIRED_DATA, requestId, RedisUtil.getLock_timeout());
            if (lock) {
                ...
                if (result > 0) {
                    //有数据设置同步时间
                    setSyncTime(RedisCacheKey.STORES_REQUIRED_TIME);
                    RedisUtil.releaseDistributedLock(redisTemplate, RedisCacheKey.STORES_REQUIRED_DATA, requestId);
                    pageResult = dao.selectAllByCache(pageRequest, RedisCacheKey.STORES_REQUIRED_DATA);
                } else {
                    setSyncTime(RedisCacheKey.STORES_REQUIRED_TIME);
                    RedisUtil.releaseDistributedLock(redisTemplate, RedisCacheKey.STORES_REQUIRED_DATA, requestId);
                    pageResult = new PageImpl<>(new ArrayList<Stores>(), pageRequest, 0);
            ...
            if (searchMap.get("searchParam").equals("null")) {
                ...
                pageResult = dao.selectCacheByConditionRequired(pageRequest, RedisCacheKey.STORES_REQUIRED_DATA, searchMap);
            }
        }
        return pageResult;
    }
    ...
    public Map<String, List<String>> selectAllCacheForExcel() {
        ...
        //取相似度匹配结果
        int compareLength = redisTemplate.llen(RedisCacheKey.STORES_COMPARE_DATA).intValue();

        if (compareLength > 0) {
            List<String> compareList = redisTemplate.lrange(RedisCacheKey.STORES_COMPARE_DATA, 0, compareLength);
        ...
        //取唯一性校验的数据
        int onlyLength = redisTemplate.llen(RedisCacheKey.STORES_ONLY_DATA).intValue();
        if (onlyLength > 0) {
            List<String> onlyData = redisTemplate.lrange(RedisCacheKey.STORES_ONLY_DATA, 0, onlyLength);
        ...
        //取必填校验的数据
        int requiredLength = redisTemplate.llen(RedisCacheKey.STORES_REQUIRED_DATA).intValue();
        if (requiredLength > 0) {
            List<String> requiredData = redisTemplate.lrange(RedisCacheKey.STORES_REQUIRED_DATA, 0, requiredLength);
    ...
    private void syncCacheData(String requestId) {
        ...
        boolean lock = RedisUtil.tryGetDistributedLock(redisTemplate, RedisCacheKey.STORES_COMPARE_DATA, requestId, RedisUtil.getLock_timeout());
        ...
        if (lock) {
            ...
            //查询全部结果
            Page<Stores> pageResult = dao.selectAllByPage(pageRequestTemp);
            //相似度比较
            ...
            //释放分布式锁
            RedisUtil.releaseDistributedLock(redisTemplate, RedisCacheKey.STORES_COMPARE_DATA, requestId);
        }
