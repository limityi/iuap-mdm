...
public class MerchantsDao {
    ...
    public Page<Merchants> selectAllByPage(PageRequest pageRequest, Map<String, Object> searchParams) {
        List<Merchants> list = merchantsRepository.selectAllData(searchParams);
    ...
    public int selectOnlyValidateData() {
        List<Merchants> resultList = merchantsRepository.selectOnlyValidateData();
        if ((!resultList.isEmpty()) && resultList.size() > 0) {
            redisTemplate.del(RedisCacheKey.STASION_ONLY_DATA);
            for (Merchants merchants : resultList
                    ) {
                redisTemplate.rpush(RedisCacheKey.STASION_ONLY_DATA, gson.toJson(merchants));
            }
    ...
    public int selectRequiredData(List<String> columns) {
        List<Merchants> resultList = merchantsRepository.selectRequiredData(columns);

        if ((!resultList.isEmpty()) && resultList.size() > 0) {
            redisTemplate.del(RedisCacheKey.STASION_REQUIRED_DATA);

            for (Merchants merchants : resultList) {
                redisTemplate.rpush(RedisCacheKey.STASION_REQUIRED_DATA, gson.toJson(merchants));
            }
            return resultList.size();
        }
        return 0;
